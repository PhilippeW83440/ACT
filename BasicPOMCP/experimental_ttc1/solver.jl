...
function search(p::POMCPPlanner, b, t::POMCPTree, info::Dict)
    ...
    # PWE
    if p.solver.dump_action_values
        action = t.a_labels[best_node]
        value = t.v[best_node]
        visited = t.n[best_node]
        println("PWE: POMCP a=$action, value=$value, visited=$visited")
    end
...
function simulate(p::POMCPPlanner, s, hnode::POMCPObsNode, steps::Int)
    ...
    if p.solver.experimental_ttc1
      o_raw = copy(o)
      o_class = get_obs_node_class_ttc1(sp[1:4], o)
      o = [o_class] # o is now a vector of a single element between 0..11 for TTC1 
    end
    ...
    if hao == 0
        if p.solver.experimental_ttc1
            hao = insert_obs_node_class!(t, p.problem, ha, o, o_raw)
        else
