...
- `experimental_ttc1::Bool`
    If `true`, runs BasicPOMCP in experimental ttc1 mode (with observations class nodes)
    default: `false`
...
@with_kw mutable struct POMCPSolver <: AbstractPOMCPSolver
    ...
    experimental_ttc1::Bool = false # PWE
...
struct POMCPTree{A,O}
    ...
    # When running BasicPOMCP-ttc1 
    #    - o_labels correspond to observations classes ID (e.g. grouped by TTC criteria) used for tree width reduction
    #    - o_labels_raw correspond to raw observations (usable by BeliefUpdater before continuous state sampling)
    ...
    o_labels_raw::Vector{O}           # actual observation corresponding to this observation node
...
function POMCPTree(pomdp::POMDP, sz::Int=1000)
    ...
    return POMCPTree{A,O}(sizehint!(Int[0], sz),
                          ...
                          sizehint!(Array{O}(undef, 1), sz), # ttc1 for o_labels_raw
...
function insert_obs_node_class!(t::POMCPTree, pomdp::POMDP, ha::Int, o, o_raw)
    push!(t.total_n, 0)
    push!(t.children, sizehint!(Int[], n_actions(pomdp)))

    push!(t.o_labels, o)
    push!(t.o_labels_raw, o_raw)
...
function insert_obs_node!(t::POMCPTree, pomdp::POMDP, ha::Int, o)
    ...
    # No distinction between obs and obs class node by default
    push!(t.o_labels, o)
    push!(t.o_labels_raw, o)
...
function updater(p::POMCPPlanner)
    println("PWE: default Particle Filter updater")
    P = typeof(p.problem)
    S = statetype(P)
    A = actiontype(P)
    O = obstype(P)
